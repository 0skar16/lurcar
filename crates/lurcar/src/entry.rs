use std::{
    cmp, collections::{HashMap, HashSet}, io::{ErrorKind, Read, Seek, Write}
};
...
impl<'a, I: Seek> Seek for Entry<'a, I> {
    fn seek(&mut self, pos: std::io::SeekFrom) -> std::io::Result<u64> {
        let out = match pos {
            std::io::SeekFrom::Start(i) => {
                ...
                self.i.inner.get_mut().seek(std::io::SeekFrom::Start(i+self.off))
            },
            std::io::SeekFrom::End(i) => {
                ...
                self.i.inner.get_mut().seek(std::io::SeekFrom::Start(((self.off+self.len) as i64 + i) as u64))
            },
            std::io::SeekFrom::Current(i) => {
                ...
                self.i.inner.get_mut().seek(std::io::SeekFrom::Current(i))
            },
        };
        if let Ok(i) = out {
            let pos = i - self.off;
...
impl<'a, W: Write> Write for Entry<'a, W> {
